...
function renderRegister(container) {
    ...
    // Toggle secret code field only for admin registration.
    document.getElementById('role-select').addEventListener('change', (e) => {
        const secretGroup = document.getElementById('secret-code-group');
        const input = secretGroup.querySelector('input');
...
async function submitQuiz(quizId, answersMap) {
    ...
    try {
        ...
        const payload = {
            quiz_id: quizId,
            responses,
            attempt_id: activeAttemptId, // Use captured ID
            proctor_logs: logs,
            trust_score: trustScore
        };
        ...
        main.innerHTML = `
            <div class="container" style="text-align:center;">
                <div class="card">
                    ...
                    <p style="margin: 1rem 0; color: #666;">
                        Transaction Hash: <br>
                        <code style="background:#eee; padding:0.2rem; border-radius:4px; word-break:break-all;">${result.transactionHash || (result.score / result.totalQuestions >= 0.6 ? 'Pending Verification' : 'Not Earned (Score < 60%)')}</code>
                    </p>
                    ${certificateBtn}
        ...
        loadAIRecommendations(document.getElementById('ai-result-analysis-std'), window.currentAttemptId);
...
async function startProctoring() {
    ...
    try {
        ...
        proctorInterval = setInterval(async () => {
            ...
            // Random Checks (legacy fallback)
            if (Math.random() > 0.95) {
                // captureProofSnapshot(video, 'Periodic Check');
            }
